    ...
    <LinearGradient colors={['#31b7ee','#d9f4fb','#eef8d8']} style={s.bg}>
      <View style={[s.page, compact && s.pageCompact]}>
        <View style={[s.top, compact && s.topCompact]}>
          <Pressable onPress={()=>router.back()} style={s.back} accessibilityLabel="חזרה" accessibilityRole="button"><Text style={s.backText}>‹</Text></Pressable>
          <View style={s.heading}>{icon ? <Image source={icon} resizeMode="contain" style={[s.headerIcon, compact && s.headerIconCompact]}/> : null}<Text numberOfLines={1} style={[s.title, compact && s.titleCompact]}>{title}</Text></View>
          <View style={s.badge}><Image source={STAR_BADGE} resizeMode="cover" style={StyleSheet.absoluteFillObject}/><Text style={s.badgeNumber}>{stars}</Text></View>
        </View>
        {voiceText ? <View style={[s.voiceRow, compact && s.voiceRowCompact]}><VoiceButton text={voiceText} compact={compact}/></View> : null}
        <View style={s.progressTrack} accessibilityLabel={`התקדמות ${Math.round(progress)} אחוז`}><View style={[s.fill,{width:`${progress}%`}]}/></View>
        <ScrollView contentContainerStyle={[s.content, compact && s.contentCompact]} showsVerticalScrollIndicator={false}>{children}</ScrollView>
      </View>
    </LinearGradient>
  </View>
}
...
export function Result({title,score,stars,againPath}: {title:string;score:number;stars:number;againPath:string}){
 return <View style={s.result}><View style={s.resultCard}><Image source={TROPHY} resizeMode="contain" style={s.trophy}/><Text style={s.resultTitle}>כל הכבוד!</Text><Text style={s.resultSub}>{title}</Text><View style={s.scorePill}><Text style={s.score}>{score}</Text><Text style={s.scoreLabel}>נקודות</Text></View><View style={s.starsRow} accessibilityLabel={`${stars} כוכבים`}>{Array.from({length:3},(_,i)=><Image key={i} source={STAR_ONLY} resizeMode="contain" style={[s.resultStar,{opacity:i<stars?1:0.22}]}/>)}</View><Pressable style={s.done} onPress={()=>router.replace(againPath as any)} accessibilityRole="button"><Text style={s.doneText}>שחק שוב</Text></Pressable><Pressable onPress={()=>router.replace('/categories')} style={s.link} accessibilityRole="button"><Text style={s.linkText}>בחר משחק אחר</Text></Pressable><Pressable onPress={()=>router.replace('/home')} style={s.link} accessibilityRole="button"><Text style={s.linkText}>חזרה לבית</Text></Pressable></View></View>
}

export function Choice({label,selected,correct,onPress}:{label:string;selected:boolean;correct?:boolean;onPress:()=>void}){return <Pressable onPress={onPress} style={[s.choice,selected&&(correct?s.good:s.bad)]} accessibilityRole="button"><Text style={s.choiceText}>{label}</Text>{selected&&<Text style={[s.mark,correct?s.goodMark:s.badMark]}>{correct?'✓':'✗'}</Text>}</Pressable>}
export function NextButton({onPress,label='המשך'}:{onPress:()=>void;label?:string}){return <Pressable onPress={onPress} style={s.nextButton} accessibilityRole="button"><Text style={s.nextText}>{label} ←</Text></Pressable>}
export function useRoundGame(total:number, correctForRound:(i:number)=>string, optionsForRound:(i:number)=>string[], id:string){const {completeGame}=useAppProgress();const [round,setRound]=useState(0),[score,setScore]=useState(0),[selected,setSelected]=useState<string|null>(null),[finished,setFinished]=useState(false);const choose=(a:string)=>{if(finished||selected)return;setSelected(a)};const next=()=>{if(finished||!selected)return;const isCorrect=selected===correctForRound(round);const finalScore=score+(isCorrect?1:0);if(round===total-1){const st=finalScore===total?3:finalScore>=Math.ceil(total*.6)?2:1;completeGame(id,st);setScore(finalScore);setFinished(true)}else{setRound(r=>r+1);setSelected(null)}};return {round,score,selected,finished,choose,next,stars:score,options:optionsForRound(round)}}
