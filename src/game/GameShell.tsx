...
export function useRoundGame(total:number, correctForRound:(i:number)=>string, optionsForRound:(i:number)=>string[], id:string){
 const {completeGame}=useAppProgress();
 ...
 const choose=(a:string)=>{if(finished || selected)return;setSelected(a)};
 const next=()=>{
   if(finished || !selected)return;
   const isCorrect=selected===correctForRound(round);
   const finalScore=score+(isCorrect?1:0);
   if(round===total-1){
     const st=finalScore===total?3:finalScore>=Math.ceil(total*.6)?2:1;
     completeGame(id,st);
     setScore(finalScore);
     setFinished(true);
   }else{
