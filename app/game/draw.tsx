...
export default function Draw(){
  ...
  const paint=(i:number)=>{
    if(finished || painted[i])return;
    const nextCount=Object.keys(painted).length+1;
    setPainted(p=>({...p,[i]:color}));
    if(nextCount>=8){completeGame('draw',2);setFinished(true);}
  };
  const targets=[
  ...
  const overlays=({scale,offsetX,offsetY}:{scale:number;offsetX:number;offsetY:number})=>Object.entries(painted).map(([i,c])=>{const [x,y]=paintSpots[Number(i)];return <View key={i} pointerEvents="none" style={{position:'absolute',left:offsetX+(x+5)*scale,top:offsetY+(y+5)*scale,width:18*scale,height:18*scale,borderRadius:9*scale,backgroundColor:c,zIndex:10}}/>});
