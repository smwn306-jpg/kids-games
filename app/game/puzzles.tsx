...
export default function Puzzles(){
 const{completeGame}=useAppProgress(); const[order,setOrder]=useState(['lion','zebra','giraffe']); const[done,setDone]=useState(false);
 const swap=(i:number)=>{if(done)return; const a=[...order];const j=a.indexOf(target[i]);[a[i],a[j]]=[a[j],a[i]];setOrder(a);if(a.join('')===target.join('')){completeGame('puzzles',3);setDone(true)}};
