...
const W=321,H=500;
export default function ProfileReady(){const {width,height}=useWindowDimensions();const sx=width/W,sy=height/H;const{name,avatar,color}=useLocalSearchParams<{name:string;avatar:string;color:string}>();const{createProfile}=useAppProgress();const avatarMap:Record<string,any>={lion:require('../assets/avatar-lion.png'),rabbit:require('../assets/avatar-rabbit.png'),elephant:require('../assets/avatar-elephant.png'),panda:require('../assets/avatar-panda.png'),monkey:require('../assets/avatar-monkey.png'),giraffe:require('../assets/avatar-giraffe.png'),fox:require('../assets/avatar-fox.png'),penguin:require('../assets/avatar-penguin.png'),bear:require('../assets/avatar-bear.png')};const avatarAsset=avatarMap[String(avatar||'lion')]||avatarMap.lion;
 useEffect(()=>{ const clean=String(name||'').trim(); if(clean) speakHebrew(`שלום ${clean}! איזה כיף לראות אותך. בואו נתחיל לשחק!`); },[name]);
