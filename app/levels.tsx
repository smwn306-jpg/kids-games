...
export default function Levels(){
 ...
 return <View style={s.stage}>
  <Image source={require('../assets/levels-reference.png')} resizeMode="stretch" style={[StyleSheet.absoluteFillObject, { width: '100%', height: '100%' }]} pointerEvents="none"/>
  <StarCountOverlay stars={stars} scaleX={sx} scaleY={sy} x={355} y={10} width={70} height={40}/>
  <Pressable style={rect(8,10,55,55)} onPress={()=>router.push('/adventure')}/>
  {levels.map((n,i)=>{const x=38+(i%4)*90,y=190+Math.floor(i/4)*96,open=n<=unlocked;return <Pressable key={n} disabled={!open} accessibilityLabel={`level-${n}`} style={rect(x,y,90,86)} onPress={()=>router.push(`/game/animals?level=${n}` as any)}><View style={[s.touch,{opacity:open?0.01:0.01}]}><Text>{open?'':'🔒'}</Text></View></Pressable>})}
