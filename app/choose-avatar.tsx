...
{avatars.map(([key,label],i)=>{const[x,y]=positions[i];return <Pressable key={key} accessibilityLabel={label} onPress={()=>setSelected(key)} style={{position:'absolute',left:(x-2)*sx,top:(y-2)*sy,width:70*sx,height:70*sy,borderRadius:16,borderWidth:selected===key?3:0,borderColor:'#71c33a'}}>{selected===key&&<Image source={require('../assets/selected-check.png')} style={{position:'absolute',right:-3*sx,top:-3*sy,width:24*sx,height:24*sy}} resizeMode="contain"/>}</Pressable>})}
